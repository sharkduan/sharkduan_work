...
| PyTorch tensor backend boundary | 46, 50-52.5 | ADR 0037 and V2 interface plan | planning boundary | adequate for now | future ADR if PyTorch objects become observable public contracts |
...
| Training checkpoint/experiment manifest policy | 50-52.5, 59 | v1 Task 25+ patterns; V2 interface plan | additive planning | adequate for now | future ADR if checkpoint identity or promotion policy becomes release-critical |
...
| 49-52.5 | no new ADR now | dataset, training loop, manifest, tuning, and full-beta harness tasks compose accepted gate and provenance patterns; Task 52.5 adds no new dependency, license, or checkpoint-payload policy |
